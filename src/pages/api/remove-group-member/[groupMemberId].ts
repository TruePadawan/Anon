...
export default async function handler(
	...
	try {
		...
		if (!clientIsAdmin) {
			...
			const memberIsNotAdmin = groupMemberData.user.id !== groupAdminProfileId;
			if (memberIsNotAdmin) {
				const deletedDoc = await prisma.groupMember.delete({
					where: {
						id: groupMemberId,
					},
                });
				...
				throw new Error("Cannot perform moderation actions on admin account");
			}
		}
	} catch (error: any) {
		res.status(500).json({
