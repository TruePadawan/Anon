...
export default function useInput(
    ...
    const [isValid, setIsValid] = useState(
        Boolean(options?.initialValueIsValid),
    );
    ...
    useEffect(() => {
        if (wasTouched) {
            ...
            const timeoutID = setTimeout(() => {
                runValidationProcess(value, validatorsRef.current).then(
                    (result) => {
        ...
        try {
            ...
            if (!valid) {
                const errorMessage: string =
                    message ?? `'${name}' validator failed`;
                messages.unshift(errorMessage);
            }
        } catch (error: any) {
            messages.unshift(
                error.message ?? `'${name}' validator threw an error`,
            );
        }
    }
    return {
